 ...
 * Tests the PDF processing pipeline with real credit report samples
 */
...
async function processCreditReport(reportPath) {
  ...
  try {
    ...
    if (result.confidence) {
      metrics.confidenceScores.push(result.confidence);
    }
    
    // Validate extraction
    const validationResult = validateExtraction(result);
    ...
    if (validationResult.accuracy >= accuracyThreshold && 
        (result.confidence === undefined || result.confidence >= confidenceThreshold)) {
      metrics.passedTests++;
      console.log(`✅ Passed: ${path.basename(reportPath)} (Accuracy: ${validationResult.accuracy.toFixed(2)}%, Confidence: ${result.confidence ? result.confidence.toFixed(2) : 'N/A'})`);
    } else {
      metrics.failedTests++;
      console.log(`❌ Failed: ${path.basename(reportPath)} (Accuracy: ${validationResult.accuracy.toFixed(2)}%, Confidence: ${result.confidence ? result.confidence.toFixed(2) : 'N/A'})`);
    ...
    return {
      filename: path.basename(reportPath),
      passed: validationResult.accuracy >= accuracyThreshold && 
              (result.confidence === undefined || result.confidence >= confidenceThreshold),
      accuracy: validationResult.accuracy,
      confidence: result.confidence,
      processingTime
...
function validateExtraction(result) {
  ...
  if (result.creditScores) {
    totalChecks++;
    const hasScores = Object.values(result.creditScores).some(bureau => 
  ...
  if (Array.isArray(result.accounts) && result.accounts.length > 0) {
    totalChecks++;
    const hasValidAccounts = result.accounts.some(account => 
      account && account.creditorName
    );
    
    if (hasValidAccounts) {
      passedChecks++;
      details.accounts = `Present (${result.accounts.length})`;
    } else {
  ...
  if (Array.isArray(result.negativeItems)) {
    totalChecks++;
    details.negativeItems = `Present (${result.negativeItems.length})`;
    passedChecks++;
  } else {
    totalChecks++;
  ...
  if (Array.isArray(result.inquiries)) {
    totalChecks++;
    details.inquiries = `Present (${result.inquiries.length})`;
    passedChecks++;
  } else {
    totalChecks++;
...
function generateSummaryReport(results) {
  ...
  // Save summary report
  const summaryPath = path.join(RESULTS_DIR, `pdf-processing-summary-${new Date().toISOString().replace(/:/g, '-')}.json`);
...
async function testPdfProcessing() {
  console.log('Starting PDF processing tests with real credit report samples...');
